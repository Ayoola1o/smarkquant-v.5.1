...
export default function LiveChart({ bot }: LiveChartProps) {
    ...
    // Update data when bot changes
    useEffect(() => {
        if (!bot || !candlestickSeriesRef.current || !equitySeriesRef.current) return;
        ...
        const candles = processData(bot.candles || []);
        let equity = [];
        if (bot.equity_snapshots) {
            equity = processData(bot.equity_snapshots.map((s: any) => ({
                time: s.timestamp, // Assuming equity_snapshots has timestamp
                value: s.equity
            })));
        }

        if (candles.length > 0) candlestickSeriesRef.current.setData(candles as CandlestickData[]);
        if (equity.length > 0) equitySeriesRef.current.setData(equity as LineData[]);

        if (bot.completed_trades && bot.completed_trades.length > 0 && markersPluginRef.current) {
            const markers = bot.completed_trades.map((t: any) => ({
                time: t.exit_time || t.entry_time, // Use exit time if available
    ...
    return (
        <div className="bg-[#0b1224] border border-slate-800 rounded-2xl p-6 shadow-2xl relative mb-6">
            <div className="flex justify-between items-center mb-6">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h2 className="text-lg font-bold text-white tracking-tight">Live Bot Chart</h2>
