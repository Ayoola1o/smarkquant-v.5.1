...
export default function SettingsPage() {
  ...
  const fetchConfig = async () => {
    ...
    try {
      ...
      toast.error(`Failed to load ${configTab}`);
    } finally {
  ...
  const handleConfigTabChange = (tab: "routes.py" | "config.py") => {
    setConfigTab(tab);
    // Fetch the new config when tab changes
    setTimeout(() => {
      setConfigLoading(true);
          ...
          <div className="bg-slate-900 rounded-lg border border-slate-700 overflow-hidden">
            <div className="flex flex-col h-[700px]">
              ...
              <div className="bg-slate-900/50 border-b border-slate-700 flex gap-4 p-4">
                <button
                  onClick={() => handleConfigTabChange("routes.py")}
                ...
                <button
                  onClick={() => handleConfigTabChange("config.py")}
                  className={`px-4 py-2 rounded-lg font-medium transition-all ${
                    configTab === "config.py"
                      ? "bg-blue-600 text-white"
                      : "bg-slate-800 text-slate-400 hover:bg-slate-700"
